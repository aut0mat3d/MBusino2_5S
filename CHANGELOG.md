...
## [2.0.0-fork] - 2026-04-17

### Added
- **JSON Profile Manager:** Custom .json profiles can now be uploaded, downloaded, and assigned directly via the Web-UI.
- **ESP-NOW Broadcast:** Added capability to broadcast sensor and M-Bus data to other ESP devices.
- **Smart Calibration UI:** New dedicated buttons to calibrate DS18B20 sensors to a Target temperature or an Average value.

### Changed
- **Massive UI/UX Overhaul:** Completely redesigned the Web-UI. It is now fully responsive, mobile-first, and uses CSS Grid/Flexbox for a clean look across all devices.
- **Fixed Webserver `%` Bug:** Completely removed the `%PRCNT%` template bug that caused layout destruction in the ESPAsyncWebServer.
- Focused the repository entirely on the 5-Slave (5S) architecture.
